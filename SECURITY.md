...
filename, size, safe DLL version metadata, and bounded SHA-256 values; it
never copies plugin binaries or recursively archives game directories.
...
personal files. Included text is opened only after a second containment and
non-reparse check immediately before the read, then validated as strict
UTF-8/UTF-16 text (with BOM support) and rejected as `RejectedUnsafeContent`
for executable/archive signatures, NUL/control bytes, or invalid encoding.
All dynamic manifest source, destination, detail, error, and scope values pass
the same centralized redaction boundary. Redaction is defense in depth, not a
claim of perfect secret detection.
...
package; collection or ZIP failures cannot produce a success result. The
output root is revalidated as non-reparse immediately before promotion.
Temporary staging is reparse-aware and cleanup failures leave a partial result
with the ZIP path and explicit action-required status rather than green
success. The output filename and destination are generated by TPM under
`SupportPackages\`, never accepted from arbitrary user input.
