...
personal files. Included text passes centralized redaction for common
credential patterns and unnecessary user-profile paths. Redaction is defense
in depth, not a claim of perfect secret detection.

The manifest records collected, absent, intentionally excluded, and failed
sources. A missing optional source does not abort the package; collection or
ZIP failures cannot produce a success result. The output filename and
destination are generated by TPM under `SupportPackages\`, never accepted
