...
Complete-TPMCertificationTransaction is the sole authority for the final outcome, over the complete evidence lifecycle, not only the pass/fail decision. It is modeled as a database transaction: it consumes immutable, already-recorded facts (the evidence ledger's append-order Sequence, each record's workflow provenance, the scorecard's own Items) rather than trusting mutable fields another code path could have set independently, and it treats publication as the final step of the same commit rather than a separate operation the caller has to keep in sync with the decision by hand.

- **Authoritative capture/evidence ledger.** `$Results.Screenshots`, appended to by `Add-Screenshot` (never by any other call site), is the one ledger the transaction reads. Every record -- captured, skipped, or failed -- carries `WorkflowId` (immutable per-run provenance, assigned once at script start) and `Sequence` (see below).
- **Capture ordering is a checkable fact, not an assumption.** Every ledger append receives a monotonic `Sequence` number at the point `Add-Screenshot` appends it -- not at the point the underlying capture function produces it, since capture and append are different events and only append order is trustworthy. The transaction asserts `final-certification-result` has a strictly higher `Sequence` than every other ledger entry, so "the final screenshot really was captured last" is enforced, not merely true because of source-code call order.
- **Derived scorecard state, not trusted scorecard fields.** `Get-TPMCertificationScoreFromItems` is the single pure function that turns a scorecard's `Items` array into `Overall`/`Passed`/`Total`/`ScorePercent`. `New-CertificationScorecard` calls it for the provisional display; `Complete-TPMCertificationTransaction` calls it again, independently, against `$Certification.Items` at commit time, for the real decision. The transaction never reads `$Certification.Overall` -- that field is a snapshot nothing prevents a future code path from leaving stale after `Items` changes; deriving fresh from the same immutable `Items` both call sites already share means the provisional display and the real commit decision can never silently diverge.
- **Publication as part of commit, not post-processing.** `Complete-TPMCertificationTransaction` takes a `-BuildArtifacts` scriptblock, invokes it with the provisional (pre-publish) transaction once evidence and score validation both complete, and publishes whatever it returns itself via `Publish-TPMCertificationArtifacts`. There is exactly one call site for `Publish-TPMCertificationArtifacts` in the whole script. A publication failure downgrades the same transaction object in place (`Passed=$false`, `Status='FAIL'`, `Overall='NOT CERTIFIED'`, `ExitCode=1`, `Published=$false`, `PublicationError=<message>`) -- there is no second, independent piece of code outside the transaction that also knows how to decide what a publish failure means. The caller only ever reads `$finalization.Published`/`.PublicationError`; it does not re-derive the outcome.
- **Commit atomicity and rollback semantics** (`Publish-TPMCertificationArtifacts`, unchanged by this round): every artifact is staged to a `.pending` file first; only once every artifact stages successfully are they all promoted via `Move-Item`; if staging or promotion fails partway, every pending file and every already-promoted file from this attempt is removed, and a pre-existing destination is never overwritten or deleted.
...
6. A passing numeric score cannot override evidence failure. Conversely, complete evidence cannot override a failed score. **Both are derived from `$Certification.Items` at commit time, never trusted off a precomputed field** (issue #154 round 2: `$Certification.Overall` was previously trusted directly).
...
10. Final report publication is guarded and is part of the same commit as the decision (issue #154 round 2), not a separate step: a write failure downgrades the transaction in place, removes partial authoritative report files, prints only a failure console outcome, and exits nonzero; a final PASS console is emitted only after all reports are written.
11. Every evidence record carries a valid positive-integer `Sequence`, assigned once, at the single point it is appended to the ledger. `final-certification-result` must have the highest `Sequence` of the entire manifest -- evidence recorded after the "final" record is a capture-ordering violation, not merely an unexpected record (issue #154 round 2).
