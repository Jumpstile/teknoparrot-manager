...
| PG-S07 | PostgreSQL elevation UX   | When install or automatic recovery requires elevation, a non-admin session receives an exact Run-as-administrator and mode-12 recovery action. No data or profile change is claimed or performed by the blocked path.                                                        | Write-PostgresAdministratorGuidance; #292 guidance tests      |
