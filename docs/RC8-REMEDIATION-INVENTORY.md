...
| UX-S08  | Health Check guided repair | Health Check reports `read-only` and `did not change anything` before offering direct, plain-language broken-path, PostgreSQL, and optional-component actions. Automatic path repair performs a dry-run candidate search, displays exact paths, asks for explicit review, saves only the reviewed set after a verified backup, validates the configured or user-selected search root as existing/canonical/non-reparse and outside TeknoParrot, TPM, and ZIP roots, offers another-folder search when no candidate is found, and hands off to AutoSync with an explicit broken-profile whitelist for re-extraction; manual repair requires explicit folder/file selection and never guesses; Details remains separate. PostgreSQL and optional-component actions are offered after broken paths are resolved or when none are present. | Invoke-LibraryHealthCheck; Show-LibraryHealthNextActions; Test-TpmRepairSearchRoot; Health Check UX and boundary tests |
...
| INV-HC-01 | Health Check inspection is read-only and does not mutate game paths, profiles, LaunchBox, PostgreSQL, ReShade, dgVoodoo2, GPU-fix, FFB, BepInEx, or controls. A selected repair/setup action is the only transition to a mutating workflow, and path repair performs a dry-run review before saving only the explicitly reviewed set after a verified backup; search roots are canonical existing non-reparse folders outside protected roots; no-candidate searches may switch to another folder, while AutoSync re-extraction is restricted to the broken profile whitelist without writing during inspection, and any real AutoSync profile-backup folder or copy failure aborts extraction, including unattended runs. | Must hold before selected action |
